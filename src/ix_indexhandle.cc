...
RC IX_IndexHandle::InsertEntry(void* pData, const RID& rid) {
    ...
    rc = rootPH.GetData((char*&)rHeader) 
     if(rc!=0)   return (rc);
...
int IX_IndexHandle::DeleteEntry(void* pData, const RID& rid) {
    ...
    bool toDelete = false;
    if ((rc = DeleteFromNode(rHeader, pData, rid, toDelete))) //从节点中删除值
        return (rc);

    // 如果删除之后树空了，设这个节点为叶子节点
    if (toDelete) {
...
RC IX_IndexHandle::SplitNode(struct IX_NodeHeader *pHeader, struct IX_NodeHeader *oldHeader, 
  ...
  PageNum newPage;  // 创建一个新节点并获取它的header
 ...
 //如果我们不拆分叶节点，那么更新新内部节点标头中的firstPageNum指针。
  if(!isLeaf){ 
